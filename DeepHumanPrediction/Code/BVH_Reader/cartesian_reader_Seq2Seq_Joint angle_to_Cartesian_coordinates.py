...
def Motion_Data_Preprocessing(time_step=150, seed_timestep=30, batch_Frame=5):
    ...
    joint_name = ['Hips', 'LeftUpLeg', 'LeftLeg', 'LeftFoot', 'LeftToeBase', 'LeftToeBaseEnd', 'RightUpLeg', 'RightLeg',
                  'RightFoot', 'RightToeBase', 'RightToeBaseEnd', 'Spine', 'Spine1', 'Spine2', 'Spine3', 'Spine4',
                  'RightShoulder',
                  'RightArm', 'RightForeArm', 'RightHand', 'RightHandEnd', 'LeftShoulder', 'LeftArm', 'LeftForeArm',
                  'LeftHand',
    ...
    for file, i in tqdm(zip(files, range(len(files)))):
        ...
        try:
            with open(file, 'r') as f:
                ...
                # 2. concatenate
                motion = np.concatenate((data['Hips'], data['LeftUpLeg'], data['LeftLeg'], data['LeftFoot'],
                                         data['LeftToeBase'], data['RightUpLeg'], data['RightLeg'],
                                         data['RightFoot'], data['RightToeBase'], data['Spine'], data['Spine1'],
                                         data['Spine2'], data['Spine3'], data['Spine4'],
                                         data['RightShoulder'], data['RightArm'], data['RightForeArm'],
                                         data['RightHand'], data['LeftShoulder'], data['LeftArm'],
    ...
    train_motion = np.reshape(motion_list,
                              (len(files), int(time_step / batch_Frame), len(motion_list[0][0]) * batch_Frame))
    train_label_motion = np.reshape(train_label_motion,
                                    (len(files), int(time_step - seed_timestep) * len(motion_list[0][0])))
    ...
    return Normalization_factor, train_motion, train_label_motion, int(seed_timestep / batch_Frame), int(
        (time_step - seed_timestep) / batch_Frame), len(train_motion[0][0]), file_name


if __name__ == "__main__":

    print('Motion_Data_Preprocessing_Starting In Main')
    Normalization_factor, train_motion, train_label_motion, seed_timestep, pre_timestep, column, file_directory = Motion_Data_Preprocessing(
        time_step=150, seed_timestep=30, batch_Frame=5)
