...
def MotionNet(order = None , epoch=None , batch_size=None , save_period=None , cost_limit=None , 
    ...
    # customized by JG
    if num_layer==1:
        d_output, d_state = d_cell.SingleLayer_feed_previous_unroll(length=pre_timestep, begin_state=e_state,
                                                                   inputs=pre_motion, merge_outputs=True,
                                                                   layout='NTC') # MultiLayer_feed_previous_unroll is also possible.
    else:
        d_output, d_state = d_cell.MultiLayer_feed_previous_unroll(length=pre_timestep, begin_state=e_state, inputs=pre_motion, merge_outputs=True, layout='NTC')



    output = mx.sym.LinearRegressionOutput(data = d_output , label=label_motion)

    digraph=mx.viz.plot_network(symbol=output,hide_weights=True)  # The diagram can be found on the Jupiter notebook.
...
if __name__ == "__main__":
    ...
    if TEST : 
        ...
        MotionNet(TEST=TEST , order=1 , save_period=1 , num_layer=num_layer , cell=cell, hidden_unit=hidden_unit , time_step = time_step , seed_timestep = seed_timestep , batch_Frame= batch_Frame , frame_time=frame_time ,graphviz=True)
        ...
        for i in range(1,len(train_file_directory)+1,1):
            ...
            completed , save_period=MotionNet(order = i , epoch=300000*start_value , batch_size=i , save_period=save_period, cost_limit=0.1 ,
