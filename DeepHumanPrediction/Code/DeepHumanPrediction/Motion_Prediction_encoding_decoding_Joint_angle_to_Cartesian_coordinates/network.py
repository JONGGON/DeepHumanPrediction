...
print("<<<Motion with encoding decoding Joint angle to Cartesian coordinates>>>")
...
def encoder(use_cudnn=True,layer_number=2,hidden_number=500,Dropout_rate=0.2):
    ...
    for i in range(layer_number):
        if use_cudnn:
            cell.add(mx.rnn.FusedRNNCell(num_hidden=hidden_number, num_layers=1, bidirectional=False, mode="lstm", prefix="lstm_encoder_{}".format(i), params=None, forget_bias=1.0, get_next_state=True))
            if Dropout_rate > 0 and (layer_number-1) > i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="lstm_dropout_encoder_{}".format(i)))
        else:
            cell.add(mx.rnn.LSTMCell(num_hidden=hidden_number, prefix="lstm_encoder_{}".format(i)))
            if Dropout_rate > 0 and (layer_number - 1) > i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="lstm_dropout_encoder_{}".format(i)))
...
'''Decoder'''
def decoder(use_cudnn=True,layer_number=2,hidden_number=500,Dropout_rate=0.2):
    ...
    for i in range(layer_number):
        ...
        if use_cudnn:
            cell.add(mx.rnn.FusedRNNCell(num_hidden=hidden_number, num_layers=1, bidirectional=False, mode="lstm", prefix="lstm_decoder_{}".format(i), params=None, forget_bias=1.0, get_next_state=True))
            if Dropout_rate > 0 and (layer_number-1) > i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="lstm_dropout_decoder_{}".format(i)))

        else:
            cell.add(mx.rnn.LSTMCell(num_hidden=hidden_number, prefix="lstm_decoder_{}".format(i)))
            if Dropout_rate > 0 and (layer_number-1) > i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="lstm_dropout_decoder_{}".format(i)))

    return cell

def MotionNet(epoch, batch_size, save_period, optimizer='sgd', learning_rate=0.1, lr_step=1000, lr_factor=0.9,stop_factor_lr=1e-08, use_gpu=True, use_cudnn=True, test=False, training_size=5, time_step=150,seed_timestep=30, batch_Frame=5, frame_time=30):

    print("-------------------Motion Net-------------------")
    '''1. Data_Loading - bvh_reader'''
    Normalization_factor, train_motion, train_label_motion , seed_timestep, pre_timestep, column, file_directory = cr.Motion_Data_Preprocessing(time_step , seed_timestep , batch_Frame)
    ...
    if test==True:
        ...
        train_motion = train_motion[:training_size]
        train_label_motion = train_label_motion[:training_size]
    ...
    #seq2seq in test
    if test==True:
        #test_premotion =  mx.sym.transpose(mx.sym.slice_axis(data=seed_motion_origin , axis=1 , begin = seed_timestep-1 , end=seed_timestep), axes=(1, 0, 2))  # (time,batch,column)
        e_out = e_output[-1]  # Shape: (1, N, C)
        e_out=mx.sym.reshape(data=e_out,shape=(1,1,-1))
        e_out = mx.sym.broadcast_to(e_out, shape=(pre_timestep, 0, 0))
        d_output, d_state = d_cell.unroll(length=pre_timestep , begin_state=e_state , inputs = e_out, merge_outputs=False, layout='TNC')

    #seq2seq in training
    else:
        e_out = e_output[-1]  # Shape: (1, N, C)
        e_out=mx.sym.reshape(data=e_out,shape=(1,batch_size,-1))
        e_out = mx.sym.broadcast_to(e_out, shape=(pre_timestep, 0, 0))
        d_output , d_state = d_cell.unroll(length=pre_timestep , begin_state=e_state , inputs = e_out, merge_outputs=False, layout='TNC')
    ...
    act1 = mx.sym.Activation(data=affine1 , act_type='relu', name='sigmoid1')
    drop1 = mx.sym.Dropout(act1 , p=Dropout_rate)
    
    affine2 = mx.sym.FullyConnected(data=drop1 , num_hidden=fc_number , name='affine2') 
    act2 = mx.sym.Activation(data=affine2 , act_type='relu', name='sigmoid2')
    ...
    #output = mx.sym.LogisticRegressionOutput(data=output, label=label_motion)
    ...
    # weights load
    cudnn_weights_path = 'weights/Cudnn_MotionNet-{}.params'.format(save_period)
    ...
    if test==False:
        for epoch in range(1, epoch + 1, 1):
            ...
            cost=np.mean(cost)
            print("Euclidean distance Error : {}".format(cost))
    ...
    if test==False:
        if os.path.exists(cudnn_weights_path) or os.path.exists(weights_path) : #FusedRNN
            ...
            bw.Motion_Data_Making(seed[:, :seed_timestep]/Normalization_factor , prediction_motion , seed_timestep , pre_timestep , batch_Frame , frame_time , file_directory , test=False)
        else:
            print("Can not test")
...
if __name__ == "__main__":
    ...
    batch_Frame=5
    save_period=10000
    file_directory=glob.glob("Data_Preprocessing/cartesian_coordinates/*.json")
    '''implement'''
    ...
    if test==False:
        ...
        for i in range(1,len(file_directory)+1,1):
            ...
            MotionNet(epoch=10000 , batch_size=i, save_period=save_period , optimizer='sgd', learning_rate=0.001 , lr_step=1000, lr_factor=0.99, stop_factor_lr=1e-08 , use_gpu=True , use_cudnn=True , test=test , training_size=i  ,time_step = time_step , seed_timestep = seed_timestep , batch_Frame= batch_Frame , frame_time=30)
    else : #test
        MotionNet(epoch=None , batch_size=None , save_period=save_period , optimizer='sgd', learning_rate=0.5 , lr_step=1000, lr_factor=0.99, stop_factor_lr=1e-08 , use_gpu=True , use_cudnn=True , test=test , training_size=None  , time_step = time_step , seed_timestep = seed_timestep , batch_Frame= batch_Frame , frame_time=30)
