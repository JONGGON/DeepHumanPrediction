...
print("<<<Motion with encoding decoding bidirectional>>>")
...
def encoder(use_cudnn=True,layer_number=2,hidden_number=500,Dropout_rate=0.2):
    ...
    for i in range(layer_number):
        if use_cudnn:
            cell.add(mx.rnn.FusedRNNCell(num_hidden=hidden_number, num_layers=1, bidirectional = True, mode="gru", prefix="encoder_{}".format(i), params=None, forget_bias=1.0, get_next_state=True))
            if Dropout_rate > 0 and (layer_number-1) >= i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="dropout_encoder_{}".format(i)))
        else:
            cell.add(mx.rnn.BidirectionalCell(l_cell=mx.rnn.GRUCell(num_hidden=hidden_number,prefix="encoder_l_cell_{}".format(i)),
                                              r_cell=mx.rnn.GRUCell(num_hidden=hidden_number,prefix="encoder_r_cell_{}".format(i)),
                                              output_prefix="bi_encoder_{}".format(i)))
            if Dropout_rate > 0 and (layer_number - 1) >= i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="dropout_encoder_{}".format(i)))
...
def decoder(use_cudnn=True,layer_number=1,hidden_number=500,Dropout_rate=0.2):
    ...
    for i in range(layer_number):
        ...
        if use_cudnn:
            cell.add(mx.rnn.FusedRNNCell(num_hidden=hidden_number, num_layers=1, bidirectional = True, mode="gru", prefix="decoder{}".format(i), params=None, forget_bias=1.0, get_next_state=True))
            if Dropout_rate > 0 and (layer_number-1) >= i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="dropout_decoder_{}".format(i)))

        else:
            cell.add(mx.rnn.BidirectionalCell(l_cell=mx.rnn.GRUCell(num_hidden=hidden_number,prefix="decoder_l_cell_{}".format(i)),
                                              r_cell=mx.rnn.GRUCell(num_hidden=hidden_number,prefix="decoder_r_cell_{}".format(i)),
                                              output_prefix="bi_decoder_{}".format(i)))
            if Dropout_rate > 0 and (layer_number-1) >= i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="dropout_decoder_{}".format(i)))
...
def MotionNet(order , epoch , batch_size , save_period, optimizer='sgd', learning_rate=0.1 , lr_step=1000, lr_factor=0.9, stop_factor_lr=1e-08 , use_gpu=True , use_cudnn=True , test=False , predict_size=5 ,time_step = 150, seed_timestep=30 , batch_Frame=5 , frame_time=30 ):
    ...
    if use_cudnn:
        rnn_output = mx.sym.Reshape(d_output[-1], shape=(-1, 2*rnn_hidden_number))
    else:
