...
class RNNCell(BaseRNNCell):
    ...
    def __init__(self, current_layer , layer_number , num_hidden , num_output , activation='tanh', shared_param = None , prefix='rnn_', params=None):
        super(RNNCell, self).__init__(prefix=prefix, params=params)
        self._num_hidden = num_hidden
        self._num_output = num_output
        self._activation = activation
        self.layer_number=layer_number # customizedRNN by JG
        self.current_layer=current_layer # customizedRNN by JG
        self.shared_param = shared_param

        if self.shared_param==None:
            self._iW = self.params.get("i2h_weight")
            self._iB = self.params.get("i2h_bias")
            self._hW = self.params.get("h2h_weight")
            self._hB = self.params.get("h2h_bias")

            # Set parameter to use residualnet when parameter sharing with encoder is not done.
            if self.current_layer < self.layer_number-1:
                self._rW = self.params.get("r_weight")
                self._rB = self.params.get("r_bias")

        else: #self.shared_param = True
            self._iW = shared_param['i2h_weight']
            self._iB = shared_param['i2h_bias']
            self._hW = shared_param['h2h_weight']
            self._hB = shared_param['h2h_bias']

        if self.current_layer == self.layer_number-1:
            linear_decoder=RNNParams("linear_decoder_")
            self._oW = linear_decoder.get('h2o_weight')  # customized by JG
            self._oB = linear_decoder.get('h2o_bias')  # customized by JG
    ...
    def __call__(self, inputs, states):
        ...
        if self.current_layer == self.layer_number-1:
            ...
            output = symbol.FullyConnected(data=output ,  num_hidden = self._num_output*2)  # customized by JG
            # Transform output size and output value to the motion network. -value : Linear , shape =(motion_shape)
            output = symbol.FullyConnected(data= output , weight = self._oW , bias=self._oB, num_hidden=self._num_output)  # customized by JG
            return output, [output]

        elif self.current_layer < self.layer_number-1 and self.shared_param==None :
            output = symbol.FullyConnected(data=output, weight=self._rW, bias=self._rB,num_hidden=self._num_output)  # customized by JG
            return output, [output]
...
class LSTMCell(BaseRNNCell):
    ...
    def __init__(self , current_layer , layer_number , num_hidden , num_output , prefix='lstm_' , shared_param = None , params=None , forget_bias=1.0):
    ...
    def __call__(self, inputs, states):
        ...
        if self.current_layer == self.layer_number-1:
            ...
            output = symbol.FullyConnected(data=next_h ,  num_hidden = 80)  # customized by JG
            # Transform output size and output value to the motion network. -value : Linear , shape =(motion_shape)
            output = symbol.FullyConnected(data=output , weight = self._oW , bias=self._oB, num_hidden=self._num_output)  # customized by JG
            return output, [next_h, next_c]

        elif self.current_layer < self.layer_number-1 and self.shared_param==None :
            output = symbol.FullyConnected(data=next_h, weight=self._rW, bias=self._rB,num_hidden=self._num_output)  # customized by JG
            return output, [next_h, next_c]
...
class GRUCell(BaseRNNCell):
    ...
    def __init__(self , current_layer , layer_number ,  num_hidden , num_output , prefix='gru_' , shared_param = None , params=None):
...
class ResidualCell(ModifierCell):
    ...
    def __call__(self, inputs, states):
        ...
        #states[0] = symbol.elemwise_add(states_out[0],states[0])
        #states[1] = symbol.elemwise_add(states_out[1],states[1])
        return output, states
