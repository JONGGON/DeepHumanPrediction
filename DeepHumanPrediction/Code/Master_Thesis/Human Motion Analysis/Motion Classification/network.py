...
def MotionNet(epoch=None , batch_size=None , save_period=None ,optimizer=None, learning_rate=None , Dropout=0.0 , use_gpu=True , use_cudnn=True , TEST=None , num_layer=None , hidden_unit=None ,time_step = None , batch_Frame= None , graphviz=None ):
    ...
    Normalization_factor, motion_data, motion_label , time_step , class_number ,column = br.Motion_Data_Preprocessing(time_step , batch_Frame , TEST)
        ...
        for epoch in range(1, epoch + 1, 1):
            ...
            for batch in train_iter:
                ...
                if epoch % 2 == 0: # Only noise is added when it is an even number
                    ...
                    #random_uniform
                    noise = mx.nd.random_uniform(low=-10 , high=10 , shape=(batch_size , time_step , column) ,ctx=ctx) # random_uniform
...
if __name__ == "__main__":
    ...
    if TEST:
        ...
        MotionNet(TEST=TEST, save_period=save_period, num_layer=num_layer, hidden_unit=hidden_unit, time_step=time_step,
                  batch_Frame=batch_Frame, use_gpu=use_gpu, use_cudnn=use_cudnn, graphviz=True)
    else:
        # batch learning
        completed = MotionNet(epoch=10000, batch_size=50, save_period=save_period, optimizer='adam',
                              learning_rate=0.001, use_gpu=use_gpu, use_cudnn=use_cudnn,
                              TEST=TEST, num_layer=num_layer, hidden_unit=hidden_unit, time_step=time_step,
                              batch_Frame=batch_Frame, graphviz=True)
        print(completed)
