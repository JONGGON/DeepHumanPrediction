...
def Motion_Data_Preprocessing(time_step=90, batch_Frame=1 ,TEST=False):
    ...
    if TEST==False:
        ...
        file_directory = glob.glob("Dataset/Test/*.bvh")
    ...
    Data = []
    motion_label=[]
    class_number=68 # Number of motion
    ...
    for i in range(len(file_directory)):
        if (i / class_number) >= 1:
            value=i-int( i / (class_number) )*class_number
            motion_label.append(value)
        else:
            motion_label.append(i)
